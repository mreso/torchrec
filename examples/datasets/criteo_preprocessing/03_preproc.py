...
from dask_utils import setup_dask
from common import (
...
def create_in_mem(input_path: str, output_path: str, day: int):
    ...
    with ExitStack() as stack:
        dense_writer = stack.enter_context(NumpyWriter(os.path.join(output_path, f'day_{day}_dense.npy'), np.dtype(np.float32), (entries, INT_FEATURE_COUNT)))
        sparse_writer = stack.enter_context(NumpyWriter(os.path.join(output_path, f'day_{day}_sparse.npy'), np.dtype(np.int32), (entries, CAT_FEATURE_COUNT)))
        label_writer = stack.enter_context(NumpyWriter(os.path.join(output_path, f'day_{day}_labels.npy'), np.dtype(np.int32), (entries, 1)))
...
if __name__ == '__main__':
    ...
    print(f'Processing took {time.time()-start_time} sec')
